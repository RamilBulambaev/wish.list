...
describe("articleDetailsSlice.test", () => {
  test("test updateProfile service pending", () => {
    ...
    const action: UnknownAction = fetchArticleById.pending("requestId", "1");

    expect(
      articleDetailsReducer(state as IArticleDetailsSchema, action)
    ).toEqual({
      error: undefined,
      isLoading: true,
    });
  ...
  test("test updateProfile service fullfield", () => {
    ...
    const action: UnknownAction = fetchArticleById.fulfilled(
      data,
      "1",
      "fulfield"
    );
